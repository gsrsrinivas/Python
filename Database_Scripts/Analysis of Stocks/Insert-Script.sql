...
update _sis.Cash_Stocks Set Batch_No = (select isnull(MAX(Batch_No)+1,1) from _sis.Cash_Stocks where batch_No <>CONVERT(int, FORMAT(GETDATE(), 'yyyyMMdd')))
...
;
--where symbol in (select symbol from _sis.Cash_Stocks except select symbol from _sis.Master_Stocks_In_Segments);
end
...
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__yearly__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = 'Yearly' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
update a set volume__quarterly__shockers = 1
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__quarterly__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = 'Quarterly' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
update a set volume__monthly__shockers = 1
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__monthly__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = 'Monthly' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
update a set volume__weekly__shockers = 1
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__weekly__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = 'Weekly' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
update a set volume__daily__shockers = 1
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__daily__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = 'Daily' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
update a set volume__4_hourly__shockers = 1
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__4_hourly__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = '4 Hourly' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
update a set volume__1_hourly__shockers = 1
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__1_hourly__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = '1 Hourly' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
update a set volume__15_minutes__shockers = 1
from _sis.Analyse_Stocks a inner join (
select symbol, 1 as volume__15_minutes__shockers from _sis.Cash_Stocks where Indicator = 'Volume' and TimeLine = '15 Minutes' and Direction = 'Shockers'
) b on b.symbol = a.Symbol
where a.Batch_No = @Batch_no
;
end 
...
-- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- select * from _sis.Analyse_Stocks where Batch_No = (select max(batch_no) from _sis.Analyse_Stocks) order by Sno;
