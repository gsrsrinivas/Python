...
# ES_Continuous, ES_System_Required, ES_Away_Mode_Required
window_flags = 0x80000000 | 0x00000001 | 0x00000040
# Get the parent directory of the current file and add to sys.path
...
def get_database_connection():
    ...
    try:
        ...
        conn_str = ('DRIVER={ODBC Driver 17 for SQL Server};SERVER=DESKTOP-EP99LTB;DATABASE=Stocks_Analysis;'
...
def insert_into_db_from_data_frame(data_frame, table_name):
    ...
    with get_database_connection() as conn:
        ...
        insert_query = f'''INSERT INTO dbo.{table_name}(sno,stock_name,symbol,bsecode,Percent_Change,price,volume,
        Indicator,TimeLine,Direction,Segments,Batch_No) VALUES (?, ?, ?, ? ,? , ?, ?, ? ,? , ?, ?, ?)'''
...
def insert_into_database_tables(table_names, bulk_file_path=None, data_frame=None):
    ...
    if bulk_file_path is not None:
        # Insert data from CSV file into the database table
        bulk_insert_from_csv(bulk_file_path, table_names[2])
    else:
        # Insert data from DataFrame into the database table
        insert_into_db_from_data_frame(data_frame, table_names[2])
    if table_names[0] != '':
...
def insert_new_columns_in_data_frame(df, tf_l_i, each_segment_list):
    ...
    df.loc[:, ['Indicator', 'TimeLine', 'Direction', 'segments']] = \
        [indicator, timeline, direction, each_segment_list]
    return df
    ...
    with requests.Session() as session:
        ...
        for attempt in range(max_retries):  # Retry logic loop
            try:  # Step 2: Submit scan data
                ...
                print(f"complete - '{str(key.replace("__", ";").replace("_", " ")).ljust(56, ' ')}'"
                      f"- {segment_val} segments ")
                return df
...
def chart_ink_file_download_and_insert_db(data_list):
    ...
    """
    segments = {'Cash': 'cash',  # 'Nifty 500':'57960','BankNifty':'136699','ETFs':'166311','Futures':'33489',
                # 'Gold ETFs':'167068','Indices':'45603','Mid-Cap 50':'136492','Nifty 100':'33619','Nifty 200':'46553',
                # 'Nifty 50':'33492', 'Nifty 500 Multi Cap 50:25:25':'1090574','Nifty and BankNifty':'109630',
                # 'Nifty Large Mid-Cap 250':'1090573','Nifty Micro Cap 250':'1090582','Nifty Mid-Cap 100':'1090585',
                # 'Nifty Mid-Cap 150':'1090588','Nifty Mid-Cap 50':'1090591','Nifty Mid-Cap Select':'1090579',
                # 'Nifty Mid-Small Cap 400':'1090575','Nifty Next 50':'1116352','Nifty Small Cap 100':'1090587',
                # 'Nifty Small Cap 250':'1090572','Nifty Small Cap 50':'1090568','Silver ETFs':'1195362',
                }
    df_all = pd.DataFrame()
    with ThreadPoolExecutor(max_workers=250) as executor:
        ...
        for data_each_list in data_list:
            ...
            for each_segment_list in segments:
                ...
                old_str = new_str
                key, data = next(iter(
                    data_each_list.items()))  # Gets the first key and iterate through the segments for one single url
                # ------ended - iterate through the segments for one single url----------------------
                # Submit the download task to the executor
                futures.append(
                    executor.submit(download_chart_ink_technical_analysis_scanner, key, data, each_segment_list))
...
def data_lists_above_daily():
    data_list1 = [
        ...
        {'ema_50_200__yearly__crosses_above': {
            'scan_clause': '( {segments_filter} ( yearly ema( yearly close , 50 ) >= yearly ema( yearly close , 200 ) ) )'}},
        {'ema_50_200__yearly__crosses_below': {
            'scan_clause': '( {segments_filter} ( yearly ema( yearly close , 50 ) <= yearly ema( yearly close , 200 ) ) )'}},
        {'ema_50_200__quarterly__crosses_above': {
            'scan_clause': '( {segments_filter} ( quarterly ema( quarterly close , 50 ) >= quarterly ema( quarterly close , 200 ) ) )'}},
        {'ema_50_200__quarterly__crosses_below': {
            'scan_clause': '( {segments_filter} ( quarterly ema( quarterly close , 50 ) <= quarterly ema( quarterly close , 200 ) ) )'}},
        {'ema_50_200__monthly__crosses_above': {
            'scan_clause': '( {segments_filter} ( monthly ema( monthly close , 50 ) >= monthly ema( monthly close , 200 ) ) )'}},
        {'ema_50_200__monthly__crosses_below': {
            'scan_clause': '( {segments_filter} ( monthly ema( monthly close , 50 ) <= monthly ema( monthly close , 200 ) ) )'}},
        {'ema_50_200__weekly__crosses_above': {
            'scan_clause': '( {segments_filter} ( weekly ema( weekly close , 50 ) >= weekly ema( weekly close , 200 ) ) )'}},
        {'ema_50_200__weekly__crosses_below': {
            'scan_clause': '( {segments_filter} ( weekly ema( weekly close , 50 ) <= weekly ema( weekly close , 200 ) ) )'}},
        {'ema_50_200__daily__crosses_above': {
            'scan_clause': '( {segments_filter} ( latest ema( latest close , 50 ) >= latest ema( latest close , 200 ) ) )'}},
        {'ema_50_200__daily__crosses_below': {
            'scan_clause': '( {segments_filter} ( latest ema( latest close , 50 ) <= latest ema( latest close , 200 ) ) )'}},
        ...
        {'Price_above_200_EMI__yearly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( yearly close / yearly ema( yearly close , 200 ) - 1 <= 0.01 and yearly close / yearly ema( yearly close , 200 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_200_EMI__quarterly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( quarterly close / quarterly ema( quarterly close , 200 ) - 1 <= 0.01 and quarterly close / quarterly ema( quarterly close , 200 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_200_EMI__monthly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( monthly close / monthly ema( monthly close , 200 ) - 1 <= 0.01 and monthly close / monthly ema( monthly close , 200 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_200_EMI__weekly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( weekly close / weekly ema( weekly close , 200 ) - 1 <= 0.01 and weekly close / weekly ema( weekly close , 200 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_200_EMI__daily__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( daily close / daily ema( daily close , 200 ) - 1 <= 0.01 and daily close / daily ema( daily close , 200 ) - 1 >= 0.00 ) ))'}},

        {'Price_below_200_EMI__yearly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( yearly close / yearly ema( yearly close , 200 ) - 1 < 0.00 and yearly close / yearly ema( yearly close , 200 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_200_EMI__quarterly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( quarterly close / quarterly ema( quarterly close , 200 ) - 1 < 0.00 and quarterly close / quarterly ema( quarterly close , 200 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_200_EMI__monthly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( monthly close / monthly ema( monthly close , 200 ) - 1 < 0.00 and monthly close / monthly ema( monthly close , 200 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_200_EMI__weekly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( weekly close / weekly ema( weekly close , 200 ) - 1 < 0.00 and weekly close / weekly ema( weekly close , 200 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_200_EMI__daily__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( daily close / daily ema( daily close , 200 ) - 1 < 0.00 and daily close / daily ema( daily close , 200 ) - 1 >= -0.01 ) ))'}},

        {'Price_above_50_EMI__yearly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( yearly close / yearly ema( yearly close , 50 ) - 1 <= 0.01 and yearly close / yearly ema( yearly close , 50 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_50_EMI__quarterly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( quarterly close / quarterly ema( quarterly close , 50 ) - 1 <= 0.01 and quarterly close / quarterly ema( quarterly close , 50 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_50_EMI__monthly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( monthly close / monthly ema( monthly close , 50 ) - 1 <= 0.01 and monthly close / monthly ema( monthly close , 50 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_50_EMI__weekly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( weekly close / weekly ema( weekly close , 50 ) - 1 <= 0.01 and weekly close / weekly ema( weekly close , 50 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_50_EMI__daily__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( daily close / daily ema( daily close , 50 ) - 1 <= 0.01 and daily close / daily ema( daily close , 50 ) - 1 >= 0.00 ) ))'}},

        {'Price_below_50_EMI__yearly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( yearly close / yearly ema( yearly close , 50 ) - 1 < 0.00 and yearly close / yearly ema( yearly close , 50 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_50_EMI__quarterly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( quarterly close / quarterly ema( quarterly close , 50 ) - 1 < 0.00 and quarterly close / quarterly ema( quarterly close , 50 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_50_EMI__monthly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( monthly close / monthly ema( monthly close , 50 ) - 1 < 0.00 and monthly close / monthly ema( monthly close , 50 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_50_EMI__weekly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( weekly close / weekly ema( weekly close , 50 ) - 1 < 0.00 and weekly close / weekly ema( weekly close , 50 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_50_EMI__daily__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( daily close / daily ema( daily close , 50 ) - 1 < 0.00 and daily close / daily ema( daily close , 50 ) - 1 >= -0.01 ) ))'}},
...
def data_lists_below_daily():
    data_list2 = [
        ...
        {'ema_50_200__4_hourly__crosses_above': {
            'scan_clause': '( {segments_filter} ( [0] 4 hour ema( [0] 4 hour close , 50 ) >= [0] 4 hour ema( [0] 4 hour close , 200 ) ) )'}},
        {'ema_50_200__4_hourly__crosses_below': {
            'scan_clause': '( {segments_filter} ( [0] 4 hour ema( [0] 4 hour close , 50 ) <= [0] 4 hour ema( [0] 4 hour close , 200 ) ) )'}},
        {'ema_50_200__1_hourly__crosses_above': {
            'scan_clause': '( {segments_filter} ( [0] 1 hour ema( [0] 1 hour close , 50 ) >= [0] 1 hour ema( [0] 1 hour close , 200 ) ) )'}},
        {'ema_50_200__1_hourly__crosses_below': {
            'scan_clause': '( {segments_filter} ( [0] 1 hour ema( [0] 1 hour close , 50 ) <= [0] 1 hour ema( [0] 1 hour close , 200 ) ) )'}},
        {'ema_50_200__15_minutes__crosses_above': {
            'scan_clause': '( {segments_filter} ( [0] 15 minute ema( [0] 15 minute close , 50 ) >= [0] 15 minute ema( [0] 15 minute close , 200 ) ) )'}},
        {'ema_50_200__15_minutes__crosses_below': {
            'scan_clause': '( {segments_filter} ( [0] 15 minute ema( [0] 15 minute close , 50 ) <= [0] 15 minute ema( [0] 15 minute close , 200 ) ) )'}},
        ...
        {'volume__4_hourly__shockers': {
            'scan_clause': '( {segments_filter} ( [0] 4 hour volume > [ 0 ] 4 hour sma( volume,20 ) * 5 ) )'}},
        {'volume__1_hourly__shockers': {
            'scan_clause': '( {segments_filter} ( [0] 1 hour volume > [ 0 ] 1 hour sma( volume,20 ) * 5 ) )'}},
        {'volume__15_minutes__shockers': {
            'scan_clause': '( {segments_filter} ( [0] 15 minute volume > [ 0 ] 15 minute sma( volume,20 ) * 5 ) )'}},
        ...
        {'double_bottom__1_hourly__setup': {
        ...
        {'Elliott_4th_Wave_Completed__4_hourly__setup': {
        ...
        {'Elliott_4th_Wave_Completed__15_minutes__setup': {
        ...
        {'Price_above_200_EMI__4_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 200 ) - 1 <= 0.01 and [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 200 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_200_EMI__1_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 200 ) - 1 <= 0.01 and [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 200 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_200_EMI__15_minutes__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 15 minute close / [0] 15 minute ema( [0] 15 minute close , 200 ) - 1 <= 0.01 and [0] 15 minute close / [0] 15 minute ema( [0] 15 minute close , 200 ) - 1 >= 0.00 ) ))'}},

        {'Price_below_200_EMI__4_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 200 ) - 1 < 0.00 and [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 200 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_200_EMI__1_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 200 ) - 1 < 0.00 and [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 200 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_200_EMI__15_minutes__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 15 minute close / [0] 15 minute ema( [0] 15 minute close , 200 ) - 1 < 0.00 and [0] 15 minute close / [0] 15 minute ema( [0] 15 minute close , 200 ) - 1 >= -0.01 ) ))'}},

        {'Price_above_50_EMI__4_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 50 ) - 1 <= 0.01 and [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 50 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_50_EMI__1_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 50 ) - 1 <= 0.01 and [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 50 ) - 1 >= 0.00 ) ))'}},
        {'Price_above_50_EMI__15_minutes__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 15 minute close / [0] 15 minute ema( [0] 15 minute close , 50 ) - 1 <= 0.01 and [0] 15 minute close / [0] 15 minute ema( [0] 15 minute close , 50 ) - 1 >= 0.00 ) ))'}},

        {'Price_below_50_EMI__4_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 50 ) - 1 < 0.00 and [0] 4 hour close / [0] 4 hour ema( [0] 4 hour close , 50 ) - 1 >= -0.01 ) ))'}},
        {'Price_below_50_EMI__1_hourly__setup': {
            'scan_clause': '( {segments_filter} ( {cash} ( [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 50 ) - 1 < 0.00 and [0] 1 hour close / [0] 1 hour ema( [0] 1 hour close , 50 ) - 1 >= -0.01 ) ))'}},
...
def purge_tables(table_part_name=''):
    ...
    sql_query = f"""DELETE FROM dbo.Cash_{table_part_name}Stocks WHERE batch_no NOT IN (SELECT distinct TOP 5 batch_no 
    FROM dbo.Cash_{table_part_name}Stocks ORDER BY batch_no DESC);
    DELETE FROM dbo.Analyse_{table_part_name}Stocks WHERE batch_no NOT IN (SELECT distinct TOP 25 batch_no 
    FROM dbo.Analyse_{table_part_name}Stocks ORDER BY batch_no DESC);"""
    cursor = conn.cursor()
    try:
